...
@section('title', 'Edit Dua | ' . $setting->title ?? 'SIS')
                        ...
                        <div class="overflow-hidden">
                            ...
                            <div class="mb-4 bg-white rounded-xl shadow-lg">
                                <div class="relative px-4 py-3 border-b flex justify-between align-middle">
                                    <h6 class="text-lg font-semibold mb-0">Edit Dua</h6>
                                    <a href="{{ route('dua.index') }}"
                                        class="btn bg-info text-white py-2 px-5 hover:bg-success rounded-md">View All</a>
                                </div>
                                <div class="p-4">
                                    <form action="{{ route('dua.update',['id' => $dua->id]) }}" method="POST">
                                        @csrf
                                        <div class="flex">
                                            <div class="basis-4/12 pe-3 mb-3">
                                                <label class="block text-sm font-medium mb-2" for="category_id">Category</label>
                                                <select class="form-select" name="category_id" id="category_id"> 
                                                    <option value="">Select Category</option>
                                                   @foreach ($categories as $cat)
                                                    <option value="{{ $cat->id }}" {{ $dua->category_id == $cat->id ? 'selected' : '' }}>{{ $cat->title }}</option>
                                                   @endforeach
                                                </select>
                                                @error('category_id')
                                        ...
                                        <div class="flex">
                                            <div class="basis-6/12 pe-3 mb-3">
                                                <label class="block text-sm font-medium mb-2" for="arabic">Arabic</label>
                                                <textarea name="arabic" id="arabic" cols="30" rows="10" placeholder="Enter Arabic" class="form-input w-full border border-gray-300 rounded p-2 bg-gray-100">{{ old('arabic', $dua->arabic) }}</textarea>
                                                @error('arabic')
                                                    <span class="text-red-500 text-sm" role="alert">
                                                        <strong>{{ $message }}</strong>
                                                    </span>
                                            ...
                                            <div class="basis-6/12 pe-3 mb-3">
                                                <label class="block text-sm font-medium mb-2" for="en_translate">English Translate</label>
                                                <textarea name="en_translate" id="en_translate" cols="30" rows="10" placeholder="Enter English Translate" class="form-input w-full border border-gray-300 rounded p-2 bg-gray-100">{{ old('en_translate', $dua->en_translate) }}</textarea>
                                                @error('en_translate')
                                                    <span class="text-red-500 text-sm" role="alert">
                                                        <strong>{{ $message }}</strong>
                                                    </span>
                                        ...
                                        <div class="">
                                            <div class="basis-6/12 pe-3 mb-3">
                                                <label class="block text-sm font-medium mb-2" for="meaning">Meaning</label>
                                                <textarea name="meaning" id="meaning" cols="30" rows="10" placeholder="Enter meaning" class="form-input w-full border border-gray-300 rounded p-2 bg-gray-100">{{ old('meaning', $dua->meaning) }}</textarea>
                                                @error('meaning')
                                                    <span class="text-red-500 text-sm" role="alert">
                                                        <strong>{{ $message }}</strong>
                                                    </span>
                                        ...
                                        <div class="mt-5 flex">
                                            <!-- Reference -->
                                            <div class="pe-3 mb-3 basis-6/12">
                                                <label class="block text-sm font-medium mb-2" for="reference">Reference</label>
                                                <input type="text" name="reference" id="reference" placeholder="Enter Reference" value="{{ old('reference', $dua->reference) }}"
                                            ...
                                            <div class="pe-3 mb-3 basis-6/12">
                                                <label class="block text-sm font-medium mb-2" for="book_name">Book Name</label>
                                                <input type="text" name="book_name" id="book_name" placeholder="Enter Book Name" value="{{ old('book_name', $dua->book_name) }}"
                                                    class="form-input w-full border border-gray-300 rounded p-2 bg-gray-100">
                                                @error('book_name')
                                                    <span class="text-red-500 text-sm" role="alert">
                                                        <strong>{{ $message }}</strong>
                                        ...
                                        <div class="mt-5 flex">
                                            <!-- Reference -->
                                            <div class="pe-3 mb-3 basis-4/12">
                                                <label class="block text-sm font-medium mb-2" for="book_no">Book No</label>
                                                <input type="number" name="book_no" id="book_no" placeholder="Enter Book No" value="{{ old('book_no', $dua->book_no) }}"
                                                    class="form-input w-full border border-gray-300 rounded p-2 bg-gray-100">
                                                @error('book_no')
                                                    <span class="text-red-500 text-sm" role="alert">
                                                        <strong>{{ $message }}</strong>
                                            ...
                                            <div class="pe-3 mb-3 basis-4/12">
                                                <label class="block text-sm font-medium mb-2" for="volume">Volume</label>
                                                <input type="text" name="volume" id="volume" placeholder="Enter Volume" value="{{ old('volume', $dua->volume) }}"
                                                    class="form-input w-full border border-gray-300 rounded p-2 bg-gray-100">
                                                @error('volume')
                                                    <span class="text-red-500 text-sm" role="alert">
                                                        <strong>{{ $message }}</strong>
                                            ...
                                            <div class="pe-3 mb-3 basis-4/12">
                                                <label class="block text-sm font-medium mb-2" for="volume">Hadith No</label>
                                                <input type="text" name="hadith_no" id="hadith_no" placeholder="Enter Book Name" value="{{ old('hadith_no', $dua->hadith_no) }}"
                                                    class="form-input w-full border border-gray-300 rounded p-2 bg-gray-100">
                                                @error('hadith_no')
                                                    <span class="text-red-500 text-sm" role="alert">
                                                        <strong>{{ $message }}</strong>
                                        ...
                                        <div class="mt-10">
                                            ...
                                            <a href="{{ route('dua.index') }}" type="button"
...
@push('scripts')
    ...
    {{-- Ck Editor --}}
    <script src="{{ asset('backend/assets/libs/tinymc/tinymce.min.js') }}"></script>
    <script>
        ...
        $(function() {
            const tinymceOptions = {
                selector: '#en_translate,#arabic, #meaning',
                menubar: false,
                statusbar: false,
                toolbar_sticky: true,
