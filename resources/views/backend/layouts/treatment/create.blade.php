...
@section('content')
    ...
    <form id="createUpdateForm" class="max-w-6xl w-full mx-auto space-y-4" enctype="multipart/form-data">
        ...
        <!-- Step 5: Select Medicines -->
        <div class="step step-5 hidden">
            <label for="medicines" class="text-lg font-semibold mb-2">Select Medicines</label>
            <select name="medicines[]" id="medicines" class="form-input w-full p-3 border-2 border-gray-300 rounded-lg text-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none" multiple>
                @foreach($medicines as $medicine)
                    <option value="{{ $medicine->id }}">{{ $medicine->title .'-'. $medicine->details->dosage }}</option>
                @endforeach
            </select>
        </div>
...
@push('scripts')
    ...
    `;
        ...
        $(document).ready(function() {
            $('#medicines').select2({
                width: '100%',  // Ensures the select box is full width
