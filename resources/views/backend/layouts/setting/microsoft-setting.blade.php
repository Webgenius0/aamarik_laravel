                    ...
                    <div class="space-y-4">
                        <form method="POST" action="{{ route('microsoft.setting.update') }}" class="max-w-6xl w-full mx-auto space-y-4">
                            ...
                            <div class="flex-col md:flex-row">
                                <h1 class="text-center text-lg">Microsoft Payment Gateway Settings</h1>
                            </div>
                            ...
                            <div class="flex flex-col md:flex-row md:items-center md:space-x-4">
                             ...
                             <!-- MS GRAPH Scope  -->
                             <div class="flex flex-col md:flex-row md:items-center md:space-x-4">
                                <label for="MS_GRAPH_SCOPE" class="text-lg font-medium mb-2 md:w-1/4">MICROSOFT GRAPH SCOPE</label>
                                <input class="form-control @error('MS_GRAPH_SCOPE') is-invalid @enderror md:w-3/4" id="MS_GRAPH_SCOPE"
                                    name="MS_GRAPH_SCOPE" placeholder="Enter your Stripe Secret" type="text"
                                    value="{{ env('MS_GRAPH_SCOPE') }}">
                                @error('MS_GRAPH_SCOPE')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                             ...
                             <!-- Ms AutoHority -->
                             <div class="flex flex-col md:flex-row md:items-center md:space-x-4">
                                <label for="MS_AUTHORITY" class="text-lg font-medium mb-2 md:w-1/4">MICROSOFT AUTHORITY</label>
                                <input class="form-control @error('MS_AUTHORITY') is-invalid @enderror md:w-3/4" id="MS_AUTHORITY"
                                    name="MS_AUTHORITY" placeholder="Enter your Stripe Secret" type="text"
                                    value="{{ env('MS_AUTHORITY') }}">
                                @error('MS_AUTHORITY')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
